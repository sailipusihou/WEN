...
export function PromoImageBadge({
  ...
  return (
    ...
    >
      <span className="font-sans text-[8px] font-bold tracking-[0.14em] text-white/85 uppercase">
        Sale
      </span>
      <span className="font-en text-sm font-semibold leading-none text-white">
        {formatPrice(convertPrice(eff.price, currency), currency)}
        {pct > 0 && (
          <span className="ml-1 font-sans text-[9px] font-medium text-white/80">-{pct}%</span>
        )}
      </span>
    </div>
  )
...
export function PromoSaleTag() {
  return (
    <span className="ml-1.5 inline-block align-middle rounded-sm bg-[#B8452E] px-1.5 py-0.5 font-sans text-[9px] font-bold tracking-[0.12em] text-white uppercase">
      Sale
    </span>
  )
}
