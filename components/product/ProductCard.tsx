  ...
  return (
    ...
    >
      <Link href={`/products/${product.id}`} className="block group" onMouseEnter={handleVideoEnter} onMouseLeave={handleVideoLeave}>
        ...
        <div className="relative aspect-[4/5] overflow-hidden bg-[#E6D8C2]">
          <OptimizedImage
            src={product.image}
        ...
        <div className="mt-3 md:mt-4 space-y-1">
          <p className="font-sans text-[8px] text-[#5F7D72]/50 tracking-[0.12em] uppercase">
            {labelFor(product.category)}
          </p>
          <h3 className="font-en text-sm md:text-base text-[#221E1A] font-medium leading-tight group-hover:text-[#5F7D72] transition-colors duration-300">
            {product.nameEn || product.name}
          </h3>
          <p className="font-sans text-[11px] text-[#57503F]/45 leading-relaxed line-clamp-1">{product.subtitleEn || product.subtitle}</p>
          {/* 评分/评论数: 有真实评价才显示 (列表页决策依据) */}
          {product.reviewCount > 0 && (
            <div className="flex items-center gap-1 pt-0.5">
              <Star size={10} className="fill-[#A07C34] text-[#A07C34]" />
              <span className="font-sans text-[10px] text-[#57503F]/60">{product.rating}</span>
              <span className="font-sans text-[10px] text-[#57503F]/35">({product.reviewCount})</span>
            </div>
          )}
          <div className="flex items-baseline gap-2 pt-0.5">
            {/* 促销价重点标注: 强调色 + SALE 标签 */}
            <span className={`font-en text-sm font-medium ${eff.discount > 0 ? 'text-[#A83420] font-semibold' : 'text-[#221E1A]'}`}>{formatPrice(convertPrice(eff.price, currency), currency)}</span>
            {eff.discount > 0 && <PromoSaleTag />}
            {(eff.originalPrice || product.originalPrice) && (
              <span className="font-sans text-[12px] text-[#57503F]/60 line-through">{formatPrice(convertPrice(eff.originalPrice || product.originalPrice || 0, currency), currency)}</span>
            )}
          </div>
        </div>
      </Link>
