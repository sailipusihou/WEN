...
export default function AddressesPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      {showForm && (
        <div className="bg-[#FFFCF7]/70 border border-otb-sand/50 rounded-sm p-6 space-y-3 mb-6">
          <select value={form.label} onChange={e => setForm(p=>({...p,label:e.target.value}))} className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans"><option>Home</option><option>Work</option><option>Other</option></select>
          <div className="grid grid-cols-2 gap-3">
            <input value={form.firstName} onChange={e => setForm(p=>({...p,firstName:e.target.value}))} placeholder="First Name *" className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans" />
            <input value={form.lastName} onChange={e => setForm(p=>({...p,lastName:e.target.value}))} placeholder="Last Name *" className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans" />
          </div>
          <input value={form.phone} onChange={e => setForm(p=>({...p,phone:e.target.value}))} placeholder="Phone" className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans" />
          <input value={form.address} onChange={e => setForm(p=>({...p,address:e.target.value}))} placeholder="Address *" className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans" />
          <div className="grid grid-cols-2 gap-3">
            <input value={form.city} onChange={e => setForm(p=>({...p,city:e.target.value}))} placeholder="City *" className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans" />
            <input value={form.state} onChange={e => setForm(p=>({...p,state:e.target.value}))} placeholder="State" className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <input value={form.zip} onChange={e => setForm(p=>({...p,zip:e.target.value}))} placeholder="ZIP Code *" className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans" />
            <select value={form.country} onChange={e => setForm(p=>({...p,country:e.target.value}))} className="w-full px-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFCF7] text-sm font-sans">
      ...
      {addresses.length === 0 && !showForm ? (
        ...
        addresses.map((addr: any) => (
          <div key={addr.id} className="bg-[#FFFCF7]/70 border border-otb-sand/50 rounded-sm p-4 flex items-start justify-between mb-3">
