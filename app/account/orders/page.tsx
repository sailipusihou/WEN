    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
      ...
      <div className="relative mb-4">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by order ID..."
          className="w-full pl-8 pr-3 py-2 border border-otb-sand/50 rounded-sm bg-[#FFFFFF] text-sm font-sans focus:outline-none focus:border-otb-terracotta/50" />
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map(order => (
            <Link key={order.id} href={"/account/orders/" + order.id} className="bg-[#FFFFFF]/70 border border-otb-sand/50 rounded-sm p-4 flex items-center justify-between hover:shadow-sm transition-shadow group">
