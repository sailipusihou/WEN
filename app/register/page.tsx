...
export default function RegisterPage() {
  ...
  return (
    <div className="min-h-[80vh] flex items-center justify-center px-4 bg-[#F1E9DC]">
      <div className="w-full max-w-md">
        ...
        <form onSubmit={handleSubmit} noValidate className="bg-white/80 border border-[#DDCEB4]/60 p-6 md:p-8 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] font-sans text-[#57503F]/60 tracking-wider uppercase mb-1.5">First Name</label>
            ...
            <div>
              <label className="block text-[10px] font-sans text-[#57503F]/60 tracking-wider uppercase mb-1.5">Last Name</label>
          ...
          <div>
            <label className="block text-[10px] font-sans text-[#57503F]/60 tracking-wider uppercase mb-1.5">Email</label>
            <input type="email" value={form.email} onChange={e => setForm(p => ({ ...p, email: e.target.value }))} required
              className="input-premium" placeholder="you@example.com" />
          </div>
          <div>
            <label className="block text-[10px] font-sans text-[#57503F]/60 tracking-wider uppercase mb-1.5">Password</label>
            <div className="relative">
              ...
              <button type="button" onClick={() => setShowPw(!showPw)}
                className="absolute right-3 top-1/2 -translate-y-1/2 text-[#57503F]/30 hover:text-[#221E1A]/60 transition-colors">
                {showPw ? <EyeOff size={16} strokeWidth={1.5} /> : <Eye size={16} strokeWidth={1.5} />}
              </button>
            </div>
          </div>
          <button type="submit" disabled={loading}
            className="w-full py-3 bg-[#221E1A] text-white text-xs tracking-[0.08em] uppercase font-sans font-medium hover:bg-[#1A1A1A] disabled:opacity-50 transition-colors">
            {loading ? "Creating..." : "Create Account"}
          </button>
          <p className="text-center text-xs font-sans text-[#57503F]/50 mt-2">
            Already registered? <Link href="/login" className="text-[#8A6A2E] hover:underline">Sign in</Link>
          </p>
        </form>
      </div>
    </div>
